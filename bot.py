...
def format_order_message(order, user_name, user_link, phone, comment, status_code=1):
    ...
    return (
        f"🚨 <b>НОВАЯ ЗАЯВКА!</b>\n"
        f"Статус: {STATUS_MAP.get(status_code, '?')}\n"
        f"➖➖➖➖➖➖➖➖➖➖\n"
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "👮‍♂️ <b>АДМИН ПАНЕЛЬ:</b>\n\n"
        "🔹 <code>/order</code> - Список заказов\n"
        "🔹 <code>/order clean</code> - Очистить базу\n"
        "🔹 <code>/order ID</code> - Выбрать заказ (для смены статуса)\n"
        "🔹 <code>/buyer</code> - Список клиентов\n"
        "🔹 <code>/clean</code> - Очистить чат\n"
        "🔹 <code>/export</code> - Скачать заказы (CSV)\n"
        "<i>Для импорта, отправьте .json с подписью /import_db</i>"
    )
...
async def cmd_order_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if oid in orders:
            ...
            await msg.reply_text(text, parse_mode=ParseMode.HTML)
        else: await msg.reply_text("❌ Не найдено.")
        return
...
async def handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = msg.text.split()
    cmd = text[0]
    update.message = update.channel_post # Хак для совместимости
    context.args = text[1:]
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith('{') and text.endswith('}'):
        try:
            ...
            await update.message.reply_text(f"🔄 Данные приняты!\n💰 {data['price']:,} руб.", reply_markup=await get_main_keyboard())
        except: pass
...
async def handle_webapp_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        context.user_data['order_data'] = data
        if 'user_comment' not in context.user_data: context.user_data['user_comment'] = 'Нет'
        await update.message.reply_text(f"✅ Расчет: {data['price']:,} руб.\n👇 Отправьте телефон.", reply_markup=await get_main_keyboard())
    except: pass
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Document.ALL, handle_document)) # Импорт

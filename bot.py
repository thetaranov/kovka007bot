...
def handle_webapp_data(update, context):
    ...
    try:
        # В старой версии python-telegram-bot данные из WebApp приходят как обычное сообщение
...
def handle_message(update, context):
    ...
    logger.info(f"💬 Обычное сообщение от {user.id}: '{text}'")
...
def main():
    ...
    logger.info("🤖 Бот запускается с поддержкой WebApp данных...")
